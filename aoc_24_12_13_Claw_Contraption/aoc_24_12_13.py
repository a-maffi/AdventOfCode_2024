...
for i in range(2,len(input),4):
    ...
    sep=input[i].find(",")
    y=input[i].find("Y")+2
    list_result_x.append(int(input[i][x:sep]))
    list_result_y.append(int(input[i][y:]))
...
for i in range(len(list_a_x)):
    ...
    if movement_b<0 or movement_b>100 or movement_b%1!=0: 
         continue
    movement_a = round((list_result_x[i] - (list_b_x[i] * movement_b))/list_a_x[i],2)
    if movement_a<0 or movement_a>100 or movement_a%1!=0: 
         continue
